...
class BertFloatClassifierModel(BertClassifierModel):
    ...
    def __call__(self, features: List[InputFeatures]) -> Union[List[int], List[List[float]]]:
        ...
        for i in range(len(pred)):
            predictions = {
                self.map2base_sentiment.get(self.all_columns[j], "neutral"): pred[i, j]
                for j in range(len(self.all_columns))
                if self.all_columns[j] in self.used_columns
            }
            best_prediction = sorted(predictions.items(), key=lambda x: x[1])[-1]
